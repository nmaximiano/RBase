import { queryDuckDB, sanitizeIdentifier, sanitizeValue } from "@/lib/db/duckdb";
...
export async function loadTableIntoR(
  ...
  const typesResult = await queryDuckDB(
    `SELECT column_name, data_type FROM information_schema.columns WHERE table_name = ${sanitizeValue(tableName)} ORDER BY ordinal_position`
  );
  ...
  const result = await queryDuckDB(`SELECT * FROM ${sanitizeIdentifier(tableName)}`);
...
export async function saveRFrameToDuckDB(
  ...
  try {
    ...
    const { importCSV, dropTable } = await import("@/lib/db/duckdb");
...
export async function execAndSync(
  ...
  if (!result.error) {
    try {
      const { listREnvironment } = await import("@/lib/r/webr");
