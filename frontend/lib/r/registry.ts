 ...
 */
import type { REnvObject } from "@/lib/r/webr";
...
export function buildRegistry(
  ...
  const unmatchedDatasets = projectDatasets.filter(
  ...
  for (const [stableId, entry] of newRegistry) {
    if (entry.datasetId) {
      ...
      if (oldEntry && oldEntry.rName !== entry.rName) {
        ...
        await localProjects.updateProjectDatasetRName(projectId, entry.datasetId, entry.rName);
