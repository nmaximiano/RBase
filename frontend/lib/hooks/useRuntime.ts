...
import { isInitialized as isWebRInitialized } from "@/lib/r/webr";
...
export function useRuntime(userId?: string): RuntimeState {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        // Start DuckDB and WebR in parallel; they're independent WASM instances
        const duckdbPromise = (async () => {
          if (!isDuckDBInitialized() || getCurrentUserId() !== userId) {
            setProgress("Loading DuckDB...");
          }
          const { initDuckDB } = await import("@/lib/db/duckdb");
          await initDuckDB(userId);
          setDuckdbReady(true);
        })();

        const webrPromise = (async () => {
          if (!isWebRInitialized()) {
            const { initWebR, setProgressCallback } = await import("@/lib/r/webr");
        ...
        await Promise.all([duckdbPromise, webrPromise]);
