...
import * as plotStorage from "@/lib/db/plotStorage";
...
export function usePlotStore(projectId: string, duckdbReady: boolean) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const saved = await plotStorage.getPlots(projectId);
        if (saved.length > 0) {
          setPlots(saved);
  ...
  const addPlots = useCallback(
    (images: ImageBitmap[], source: "user" | "agent", code?: string) => {
      ...
      if (newPlots.length > 0) {
        ...
        plotStorage.savePlots(projectId, newPlots).catch((e) =>
  ...
  const removePlot = useCallback(
    (plotId: string) => {
      ...
      plotStorage.deletePlot(projectId, plotId).catch((e) =>
  ...
  const clearPlots = useCallback(() => {
    setPlots([]);
    plotStorage.clearPlots(projectId).catch((e) =>
