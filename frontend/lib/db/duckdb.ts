/**
 * Singleton DuckDB-WASM connection with OPFS persistence.
 *
 * This module owns the only connection the app uses. It wires DuckDB-WASM
 * to a file in OPFS (`opfs://RBase_<userId>_v2.duckdb`) so the database
 * survives reloads. The `_v2` suffix is a schema-version gate: bump it
 * (and the path) when a schema change can't be expressed as an idempotent
 * `ALTER`. Storage is per-user; `initDuckDB(userId)` tears down and
 * re-opens if the user id changes so accounts don't read each other's
 * data. If OPFS isn't available (private mode, etc.) we fall back to an
 * in-memory DB and fire a `duckdb-opfs-fallback` event.
 *
 * Key functions:
 *   - initDuckDB: idempotent init; namespaces OPFS by userId
 *   - queryDuckDB: run SQL, auto-schedules a checkpoint on writes
 *   - importCSV / importParquet: register bytes as a table
 *   - clearAllData: drops tables, terminates the DB, deletes OPFS entries
 *   - sanitizeIdentifier / sanitizeValue: the SQL safety primitives
 *
 * Tables: `_projects`, `_project_datasets`, `_datasets`, `_chat_history`,
 * `_r_code_history`, `_rdata_blobs`, `_plot_store`, `_r_scripts`,
 * `_user_preferences`, plus transient `_rview_<stableId>` views that
 * mirror live R data frames for the table UI. Every identifier and
 * string literal interpolated into SQL in this repo MUST go through the
 * sanitizers; see CONTRIBUTING.md.
 */

import * as duckdb from "@duckdb/duckdb-wasm";
import jschardet from "jschardet";

/** Sanitize a SQL identifier (table/column name): strip unsafe chars, wrap in double quotes. */
export function sanitizeIdentifier(name: string): string {
  const clean = name.replace(/[^a-zA-Z0-9_ ]/g, "");
  return `"${clean}"`;
}

/** Sanitize a SQL string value: escape single quotes by doubling, wrap in single quotes. */
...
async function createTables(c: duckdb.AsyncDuckDBConnection): Promise<void> {
  await c.query(`
  ...
  await c.query(`
    CREATE TABLE IF NOT EXISTS _project_datasets (
  ...
  await c.query(`
    CREATE TABLE IF NOT EXISTS _chat_history (
      ...
      PRIMARY KEY (project_id, turn_index)
    )
  `);
  ...
  await c.query(`
    CREATE TABLE IF NOT EXISTS _r_scripts (
...
/** Run schema migrations for existing databases (idempotent ALTER statements). */
async function runMigrations(c: duckdb.AsyncDuckDBConnection): Promise<void> {
  // Add plots column to _chat_history (for existing installs)
  try {
    await c.query(`ALTER TABLE _chat_history ADD COLUMN plots TEXT`);
  } catch {
    // Column already exists, ignore
  }

  // Add r_name column to _project_datasets if missing
  try {
    await c.query(`ALTER TABLE _project_datasets ADD COLUMN r_name VARCHAR`);
  } catch {
    // Column already exists, ignore
  }
}
  ...
  initPromise = (async () => {
    ...
    if (opfsPath) {
      try {
        ...
        if (typeof window !== "undefined") {
          window.dispatchEvent(
            new CustomEvent("duckdb-opfs-fallback")
          );
        }
      }
    } else {
...
export async function importCSV(
  ...
  const safeId = sanitizeIdentifier(tableName);
  const safeVal = sanitizeValue(tableName);

  await d.registerFileBuffer(`${tableName}.csv`, csvBytes);
  try {
    await c.query(
      `CREATE OR REPLACE TABLE ${safeId} AS SELECT * FROM read_csv_auto('${tableName}.csv')`
    );
  } catch (e) {
    const msg = String(e).toLowerCase();
    if (msg.includes("unicode") || msg.includes("utf") || msg.includes("encoding") || msg.includes("invalid input")) {
      ...
      try {
        ...
        await c.query(
          `CREATE OR REPLACE TABLE ${safeId} AS SELECT * FROM read_csv_auto('${tableName}.csv', ignore_errors=true)`
        );
      }
    } else {
      await d.dropFile(`${tableName}.csv`);
  ...
  const metaResult = await c.query(
    `SELECT column_name FROM information_schema.columns WHERE table_name = ${safeVal} ORDER BY ordinal_position`
  );
  ...
  const countResult = await c.query(
    `SELECT count(*)::INTEGER as cnt FROM ${safeId}`
  );
...
export async function importParquet(
  ...
  await c.query(
    `CREATE OR REPLACE TABLE ${safeId} AS SELECT * FROM read_parquet('${tableName}.parquet')`
  );
...
export async function getTableRows(
  ...
  const typesResult = await c.query(
    `SELECT column_name, data_type FROM information_schema.columns WHERE table_name = ${safeVal} ORDER BY ordinal_position`
  );
  ...
  if (dateCols.size > 0 || tsCols.size > 0) {
    ...
    selectExpr = allCols.map((col) => {
      ...
      if (dateCols.has(col)) return `CAST(${safeCol} AS VARCHAR) AS ${safeCol}`;
      if (tsCols.has(col)) return `STRFTIME(${safeCol}, '%Y-%m-%d %H:%M:%S') AS ${safeCol}`;
      return safeCol;
    }).join(", ");
  }

  const orderClause =
    sortCol ? `ORDER BY ${sanitizeIdentifier(sortCol)} ${sortDir === "desc" ? "DESC" : "ASC"}` : "";

  const dataResult = await c.query(
    `SELECT ${selectExpr} FROM ${safeId} ${orderClause} LIMIT ${perPage} OFFSET ${offset}`
  );
...
export async function dropTable(tableName: string): Promise<void> {
  const c = getConn();
  await c.query(`DROP TABLE IF EXISTS ${sanitizeIdentifier(tableName)}`);
  scheduleCheckpoint();
}
...
export async function tableExists(tableName: string): Promise<boolean> {
  ...
  const result = await c.query(
    `SELECT count(*)::INTEGER as cnt FROM information_schema.tables WHERE table_name = ${sanitizeValue(tableName)}`
  );
...
export function isOpfsFallback(): boolean {
  return _opfsFallback;
}
    ...
    try {
      ...
      if (nameVec) {
        for (let i = 0; i < tables.numRows; i++) {
          ...
          try { await conn.query(`DROP TABLE IF EXISTS ${sanitizeIdentifier(name)}`); } catch (e) {
